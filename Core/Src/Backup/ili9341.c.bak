...
void LCD_Rect_Fill(uint16_t x, uint16_t y, uint16_t w, uint16_t h,
	...
	uint16_t color = H24_RGB565(1, color24);
...
void LCD_FontRw(uint16_t x, uint16_t y, const char *text, const char *textRw,
		const GFXfont *p_font, uint8_t size, uint32_t color24,
		uint32_t color24Rw) {
	int16_t cursor_x = x;
	int16_t cursor_y = y;
	int16_t cursor_x_Rw = x;
	int16_t cursor_y_Rw = y;
	uint16_t lenght = 0;
	uint16_t len_text = strlen(text);
	uint16_t len_textRw = strlen(textRw);
	GFXfont font;
	memcpy(&font, p_font, sizeof(GFXfont));
	if (len_text > len_textRw)
		lenght = len_text;
	else
		lenght = len_textRw;
	for (uint16_t text_pos = 0; text_pos < lenght; text_pos++) {
		char c = text[text_pos];
		char cRw = textRw[text_pos];
			GFXglyph glyphRw;
			GFXglyph glyph;
			memcpy(&glyphRw, &font.glyph[cRw - font.first], sizeof(GFXglyph));
			memcpy(&glyph, &font.glyph[c - font.first], sizeof(GFXglyph));
		if (cRw == '\n') {
			cursor_x_Rw = x;
			cursor_y_Rw += font.yAdvance * size;
		} else if (cRw >= font.first && cRw <= font.last && cRw != '\r') {
			if (text_pos < len_textRw) {
				if (c != cRw || len_textRw > len_text)
					LCD_Char(cursor_x_Rw, cursor_y_Rw, &glyphRw, &font, size,
							color24Rw);
			}
			cursor_x_Rw += glyphRw.xAdvance * size;
		}
		if (c == '\n') {
			cursor_x = x;
			cursor_y += font.yAdvance * size;
		} else if (c >= font.first && c <= font.last && c != '\r') {
			if (text_pos < len_text) {
				if (c != cRw || len_text > len_textRw || glyphRw.xAdvance < glyph.xAdvance)
					LCD_Char(cursor_x, cursor_y, &glyph, &font, size, color24);
			}
			cursor_x += glyph.xAdvance * size;
		}
...
void LCD_Bitmap_Mono(uint16_t x, uint16_t y, const char *bitmap,
	...
	for (uint16_t i = 0; i < h; i++) {
		for (uint16_t j = 0; j < w; j++) {
			...
			if (byte & (1 << (bit_pos % 8)))
				LCD_Data_16(H24_RGB565(1, color24_set));
			else
				LCD_Data_16(H24_RGB565(1, color24_unset));
			bit_pos++;
		}
	}
}
